...
if __name__ == '__main__':
    ...
    if len(compare_tuples) > 0:
        for file_a, file_b in compare_tuples:
            frame_a = pd.read_csv(file_a, nrows=1000, delimiter=';', header=None)
            #print(frame_a.head())
            frame_b = pd.read_csv(file_b, nrows=1000)
            #print(frame_b.head())

            print(np.allclose(frame_a.iloc[:, 1:], frame_b.iloc[:, 1:], rtol=1e-3))
            # -> there is only a very small discrepancy between OTBiolab Exporting and Python Exporting (1e-3...1e-4)
